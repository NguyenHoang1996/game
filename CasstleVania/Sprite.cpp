...
Sprite::Sprite( LPWSTR filePath, int totalFrames, int cols)
{
	trace("create Sprite", 0);
	...
	auto rs = _texture.loadFromFile(filePath);
	if (rs != D3D_OK)
		throw;
	...
	//create surface
	G_Device->CreateOffscreenPlainSurface(
		G_ScreenWidth,
		G_ScreenHeight,
		D3DFMT_X8R8G8B8,
		D3DPOOL_DEFAULT,
...
void Sprite::render( Viewport* viewport)
{
	...
	RECT r;
	r.top = G_ScreenWidth - _bound.top;
	r.left = _bound.left;
	r.bottom = G_ScreenHeight - _bound.bottom;
	r.right = _bound.right;

	G_Device->ColorFill(_surface, NULL, D3DXCOLOR(1.0f, 0.0f, 1.0f, 1.0f));
